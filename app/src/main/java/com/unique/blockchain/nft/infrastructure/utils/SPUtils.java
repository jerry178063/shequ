...
public class SPUtils {
     ...
     * @param keyword
     */
    public void saveCollect(String keyword) {
        // 获取搜索框信息
        SharedPreferences mysp = mContext.getSharedPreferences("search_history_collect", 0);
        String old_text = mysp.getString("history_collect", "");
        // 利用StringBuilder.append新增内容，逗号便于读取内容时用逗号拆分开
        StringBuilder builder = new StringBuilder(old_text);
        builder.append(keyword + ",");
        ...
        if (!old_text.contains(keyword + ",")) {
            SharedPreferences.Editor myeditor = mysp.edit();
            myeditor.putString("history_collect", builder.toString());
            myeditor.commit();
        }
    }

    public String[] getHistoryListCollect() {
        // 获取搜索记录文件内容
        SharedPreferences sp = mContext.getSharedPreferences("search_history_collect", 0);
        String history = sp.getString("history_collect", "");
        // 用逗号分割内容返回数组
        String[] history_arr = history.split(",");
        // 保留前50条数据
        if (history_arr.length > 50) {
     ...
     * @param keyword
     */
    public void saveMe(String keyword) {
        // 获取搜索框信息
        SharedPreferences mysp = mContext.getSharedPreferences("search_history_me", 0);
        String old_text = mysp.getString("history_me", "");
        // 利用StringBuilder.append新增内容，逗号便于读取内容时用逗号拆分开
        StringBuilder builder = new StringBuilder(old_text);
        builder.append(keyword + ",");
        ...
        if (!old_text.contains(keyword + ",")) {
            SharedPreferences.Editor myeditor = mysp.edit();
            myeditor.putString("history_me", builder.toString());
            myeditor.commit();
        }
    }

    public String[] getHistoryListMe() {
        // 获取搜索记录文件内容
        SharedPreferences sp = mContext.getSharedPreferences("search_history_me", 0);
        String history = sp.getString("history_me", "");
        // 用逗号分割内容返回数组
        String[] history_arr = history.split(",");
        // 保留前50条数据
        if (history_arr.length > 50) {
     ...
     * 清除搜索记录
     */
    public void cleanHistoryCollect() {
        SharedPreferences sp = mContext.getSharedPreferences("search_history_collect", 0);
        SharedPreferences.Editor editor = sp.edit();
        editor.clear();
        editor.commit();
