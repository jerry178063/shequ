...
import com.unique.blockchain.nft.adapter.AllAsstesFailAdapter;
import com.unique.blockchain.nft.adapter.EthAllAsstesFailAdapter;
...
public class EthFailureFragment extends BaseFragment {
    ...
    private EthAllAsstesFailAdapter allAsstesAdapter=new EthAllAsstesFailAdapter(R.layout.all_fail_assets_item,new ArrayList<>());
...
//                    .upJson(map.toString())
                ...
                .execute(new JsonCallback<TransactionRecordNewBean>() {
                    ...
                    public void onSuccess(TransactionRecordNewBean versionInfo, Call call, Response response) {
                        Log.e("FFKK3","失败:" + new Gson().toJson(versionInfo));
                        if(smr != null) {
                            smr.finishRefresh();
                        }
                        dismissDialog();
...
//                                    list.addAll(versionInfo.getResult().getRecords());
                                    ...
                                    page++;
                                    Log.e("FFKK3","TextUtils.isEmpty(list2.toString()):" + list2.size());
