...
public class AESEncrypt {
    ...
    // byte 转成 32位
    public static byte [] bytetoString (char [] string){
        byte []bytes=new byte[string.length];
        for (int i = 0; i < string.length/2; i++){
            byte value = (byte) (string[2 * i]  << 4);
            value += string[2*i + 1];
    ...
    // byte 转成 32位
    public static byte [] bytetoString1 (char [] bytes){

        byte[] chars = new byte[bytes.length];
        for ( int i = 0; i < bytes.length; i++) {
            chars[0] = (byte) ((bytes[i] & 0xf0) >> 4);

            chars[i+1] = (byte) (bytes[i] & 0x0f);
        }
        return chars;
    }
