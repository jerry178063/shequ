 ...
 * 票务---更多
 * */
public class MarkTicketActivity extends BaseActivity implements IMarkTicketCallBack {
    ...
    public void initUI() {
        ...
        try {
            unique = userDao.queryBuilder().where(UserDao.Properties.Name.eq(SPUtils.getString(MarkTicketActivity.this, Tools.name, ""))).build().unique();
        } catch (Exception e) {
        }
        if (unique != null) {
        ...
        markTicketAdapter.setOutPriceListener(new CommAdapter.OutPrice() {
            ...
            public void outPrice(int position) {
                ...
                if(rows.get(position).getSellMode() == 1){//购买
                    Intent intent = new Intent(MarkTicketActivity.this, BuyMarkActivity.class);
                    ...
                    Intent intent = new Intent(MarkTicketActivity.this, OutPriceActivity.class);
                    intent.putExtra("title",rows.get(position).getNftName());
                    intent.putExtra("nftId",rows.get(position).getNftId());
                    intent.putExtra("uniqueAdress",uniqueAdress);
                    intent.putExtra("productType",rows.get(position).getProductType() + "");
        ...
        smr.setOnRefreshListener(new OnRefreshListener() {
            ...
            public void onRefresh(@NonNull @NotNull RefreshLayout refreshLayout) {
                if(iMarkTicketPresenter != null) {
                    showDialog();
                    page = 1;
                    iMarkTicketPresenter.getData(page, 10, "","1");
                }
            }
        });
