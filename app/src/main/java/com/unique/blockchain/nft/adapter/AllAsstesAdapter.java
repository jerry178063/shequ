...
public class AllAsstesAdapter extends BaseQuickAdapter<TransactionRecordNewBean.Result.Records, BaseViewHolder> {
    private String walletAddress;
    List<TransactionRecordNewBean.Result.Records> data;
    public AllAsstesAdapter(int layoutResId, @Nullable @org.jetbrains.annotations.Nullable List<TransactionRecordNewBean.Result.Records> data) {
    ...
    protected void convert(BaseViewHolder helper, TransactionRecordNewBean.Result.Records item) {
        ...
        if(!item.getAmount().contains("-")) {//转入
            tv_event.setText("转入");
            tv_num.setText( "+" + String.format("%.6f", Double.valueOf(item.getAmount())) + " UNIQUE");
