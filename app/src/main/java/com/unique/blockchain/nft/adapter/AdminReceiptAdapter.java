 ...
 * 我的-交易中
 */
public class AdminReceiptAdapter extends BaseQuickAdapter<MarkHotBean.Rows, BaseViewHolder> {
    private String type = "";
    private int getType = 0;
    private int mPosition;
    private CommAdapter.OutPrice mOutPrice;
    private SparseArray<CountDownTimer> timerArray;
    private AdminGetHuo adminGetHuo;

    public interface AdminGetHuo {
        void adminGetHuo(int position, MarkHotBean.Rows item);
    }
    ...
    public void setAdminGetHuo(AdminGetHuo adminGetHuo) {
        this.adminGetHuo = adminGetHuo;
    }
    ...
    public AdminReceiptAdapter(int layoutResId, @Nullable @org.jetbrains.annotations.Nullable List<MarkHotBean.Rows> data) {
...
//        tv_detail_sends = helper.getView(R.id.tv_detail_sends);
        ...
        ImageView img_admin_shouhuo = helper.getView(R.id.img_admin_shouhuo);
        ...
        img_admin_shouhuo.setOnClickListener(new View.OnClickListener() {
            @Override
            public void onClick(View v) {
//                    if (item.getTakeState() == 0 || item.getTakeState() == 1) {
//                        ToastUtil.showShort(mContext, "未提货");
//                        return;
//                    } else if (item.getTakeState() == 1) {
//                        ToastUtil.showShort(mContext, "提货中");
//                        return;
//                    }
                    if (adminGetHuo != null) {
                        adminGetHuo.adminGetHuo(helper.getLayoutPosition(), item);
                    }
            }
        });
...
//                mContext.startActivity(intent);
        ...
        TextView tv_tihuo_status = helper.getView(R.id.tv_tihuo_status);//提货状态
//        if (item.getTakeState() == 0) {
//            tv_tihuo_status.setText("未提货");
//        } else if (item.getTakeState() == 1) {
//            tv_tihuo_status.setText("提货中");
//        } else if (item.getTakeState() == 2) {
//            tv_tihuo_status.setText("已提货");
//        }
    }
