 ...
 * 合约转账
 */
public class TransferHeYueWalletAssetsActivity extends BaseActivity implements SafeAdminDialog.OnSafeClickView, TransferSuccessDialog.OnBackZhujiClickView {
    ...
    private String wallet_address,heyue_type;
                    ...
                    public void onSuccess(EthZiChanInfo jsonObject, Call call, Response response) {
                        if(jsonObject.getCode() == 200) {
                            ...
                            if(jsonObject.getResult() != null && jsonObject.getResult().size() > 0){
                                if(!TextUtils.isEmpty(heyue_type)){
                                    for (int i = 0 ; i < jsonObject.getResult().size(); i++){
                                        if(heyue_type.contains(jsonObject.getResult().get(i).getTokenSymbol())) {
                                            contractAddress = jsonObject.getResult().get(i).getContractAddress();
                                            unit = jsonObject.getResult().get(i).getUnit();
                                        }
                                    }
...
//                        startActivityForResult(intent, 1);
            ...
            case R.id.img_saoyisao://扫一扫
                Intent intent_sao = new Intent(TransferHeYueWalletAssetsActivity.this, CaptureActivity.class);
...
//                        }
                if (TextUtils.isEmpty(ed_scan_content.getText().toString())) {
                    ToastUtils.showToast(TransferHeYueWalletAssetsActivity.this, "请输入接收地址");
                    return;
                }
                if (ed_scan_content.getText().toString().trim().equals(address)) {
                    ToastUtils.showToast(TransferHeYueWalletAssetsActivity.this, "地址错误，请重新输入");
                    return;
                }
                if (wallet_name != null) {
                    if (wallet_name.equals("UNIQUE")) {
                ...
                if (!TextUtils.isEmpty(et_num.getText().toString())) {
                    if (!TextUtils.isEmpty(amount)) {
                        if(!TextUtils.isEmpty(wallet_name)){
                            if(wallet_name.equals("UNIQUE")) {
                                if (new BigDecimal(et_num.getText().toString()).compareTo(new BigDecimal(0)) == 0) {
                                    ToastUtils.showToast(TransferHeYueWalletAssetsActivity.this, "输入的金额不能为0");
                                    return;
                                } else if (Double.parseDouble(et_num.getText().toString()) > Double.parseDouble(String.format("%.6f", Double.valueOf(amount) / 1000000))) {
                                    ToastUtils.showToast(TransferHeYueWalletAssetsActivity.this, "转账数量大于可用余额");
                                    return;
                                }
                            }else if(wallet_name.equals("ETH")) {
                                if (new BigDecimal(et_num.getText().toString()).compareTo(new BigDecimal(0)) == 0) {
                        ...
                        ToastUtils.showToast(TransferHeYueWalletAssetsActivity.this, "可用余额为0");
                        return;
                    }
                } else {
                    ToastUtils.showToast(TransferHeYueWalletAssetsActivity.this, "转账数量不能为空");
                    return;
                }
                    ...
                    if (wallet_name.equals("UNIQUE")) {
                        if (ed_scan_content.getText().toString().trim().startsWith("unique")) {
                            if (Double.parseDouble(et_num.getText().toString()) > ((Double.parseDouble(String.format("%.6f", Double.valueOf(amount) / 1000000)) - Double.parseDouble(tv_icon_more.getText().toString().replace("UNIQUE", "").trim())))) {
                                ToastUtil.showShort(TransferHeYueWalletAssetsActivity.this, "矿工费不足");
                                return;
                            }
                            getKeyCode();
                        }
...
//                                    handler.sendEmptyMessage(123);
                                    if (transferSuccessDialog == null) {
                                        transferSuccessDialog = new TransferSuccessDialog(TransferHeYueWalletAssetsActivity.this);
                                        transferSuccessDialog.setOnclick(TransferHeYueWalletAssetsActivity.this);
                                    ...
                                    ToastUtils.showToast(TransferHeYueWalletAssetsActivity.this, "转账失败");
                                    finish();
                                }
...
//        }
        ...
        if (pass_sha.equals(shapsd)) {
            ...
            ToastUtils.showToast(TransferHeYueWalletAssetsActivity.this, "请输入正确的安全密码");
            return;
        }
        safeAdminDialog.dismiss();
    }
    ...
    private void standTrans(EthResultBean ethResultBean) {
        ...
        EthWallet.ETHTxCommonParams ethTxCommonParams = new EthWallet.ETHTxCommonParams(eth_gas,
                eth_limit, new BigDecimal(result.getChainId()).toPlainString(), contractAddress
        );
        BigDecimal multiply = new BigDecimal(et_num.getText().toString()).multiply(new BigDecimal("10").pow(Integer.parseInt(unit + ""))).setScale(0,BigDecimal.ROUND_HALF_DOWN);
//        String s = ethWallet.EthereumTransactionSigning(ethTxCommonParams, multiply.stripTrailingZeros().toPlainString(), new BigDecimal(result.getNonce()).toPlainString());
        String s = ethWallet.EthereumERC20Signing(ethTxCommonParams, multiply.stripTrailingZeros().toPlainString(), new BigDecimal(result.getNonce()).toPlainString(),ed_scan_content.getText().toString());
                    ...
                    public void onSuccess(EthTongLianTransferBean jsonObject, Call call, Response response) {
                        ...
                        if (jsonObject.getCode() == 200 && jsonObject.getResult() != null) {
                            if(jsonObject.getResult().getHash() != null && jsonObject.getResult().getHash().startsWith("0x")){
                                ...
                                ToastUtil.showShort(TransferHeYueWalletAssetsActivity.this,"转账失败");
                            }
                        }else {
                            ToastUtil.showShort(TransferHeYueWalletAssetsActivity.this,"转账失败");
                        }
